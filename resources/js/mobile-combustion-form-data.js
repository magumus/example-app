...
$('#storeFormData').on('click', function(){
	...
	$('#mobileCombustionForm select').each(function(){
		var errorid = $(this).attr("id");
	...
	var amount_of_activity = $('#amount_of_activity').val();
	var units = $('#units').val();
	if(amount_of_activity && units != 0 && units){ $('.amountoffuelbox').removeAttr('style'); }else{ $('.amountoffuelbox').css('border', '1px solid red'); error = 1; }
...
$('#saveDataConfirmYes').on('click', function(){
	...
	$('#mobileCombustionForm select').each(function(){
		...
		if(id == 'units'){
			var amount_of_activity = $('#amount_of_activity').val();
			if(editRowNumber){
				$("#amount_of_activity"+editRowNumber).find('span').text(amount_of_activity);
				$("#"+id+editRowNumber).find('span').text(val);
			}else{
				row += '<td data-th="Amount of fuel" data-id="amount_of_activity" id="amount_of_activity'+rowNumber+'"><span class="bt-content">'+amount_of_activity+'</span></td>';
	...
	$('#mobileCombustionForm input').each(function(){
		...
		if(val != 'Sıfırla' && val != 'Kaydet' && id != 'amount_of_activity'){ 
			if(editRowNumber){
				$("#"+id+editRowNumber).find('span').text(val);
			}else{
				row += '<td data-th="'+txt+'" data-id="'+id+'" id="'+id+rowNumber+'"><span class="bt-content">'+val+'</span></td>';
...
function resetFormData(){
	$('#mobileCombustionForm').trigger("reset");
